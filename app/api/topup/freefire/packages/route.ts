...
export async function POST(req: Request) {
  ...
  const { data: result, error: rpcError } = await supabaseAdmin.rpc('deduct_balance', {
    p_user_id:     user.id,
    p_amount:      pkg.price,
    p_description: 'เติม Free Fire ' + pkg.name + ' Player ID: ' + playerId,
  ...
  await supabaseAdmin.from('notifications').insert({
    ...
    message:    'Free Fire ' + pkg.name + ' Player ID: ' + playerId,
    message_th: 'Free Fire ' + pkg.name + ' Player ID: ' + playerId,
    is_read:    false,
  })
